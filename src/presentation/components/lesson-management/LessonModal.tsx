...
export const LessonModal: React.FC<LessonModalProps> = ({
  ...
  const insertTemplate = (type: 'manual' | 'code' | 'theory') => {
    let templateText = '';
    if (type === 'manual') {
...
Consulta los enlaces oficiales para profundizar en los conceptos avanzados.`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm animate-in fade-in duration-200 overflow-y-auto">
      <div className="w-full max-w-2xl border-2 border-slate-950 bg-white dark:bg-slate-900 p-6 text-slate-950 dark:text-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_#00b835] my-8 animate-in zoom-in-95 duration-200">
        <h2 className="font-display text-xl font-black text-slate-950 dark:text-white mb-4 pb-2 border-b-2 border-slate-950 dark:border-slate-800 flex justify-between items-center">
          <span>{isEditing ? '✏️ Editar Tema / Lección' : '➕ Crear Nuevo Tema / Manual'}</span>
          <span className="text-xs font-mono bg-[#00cc33] text-slate-950 px-2.5 py-0.5 border border-slate-950 font-extrabold shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]">TEMA</span>
        </h2>
